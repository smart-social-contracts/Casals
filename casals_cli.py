...
def _run_icp_deploy(args, mode: str = "upgrade") -> None:
    if mode == "install":
        # A create used to deploy casals_backend + ic_file_registry first,
        # because the browse UI build baked VITE_CANISTER_ID from a live
        # ic_file_registry mapping and so could not run before that id existed.
        # The UI now reads its backend id from the asset canister's ic_env
        # cookie, so no build needs the id and one deploy is enough.
        #
        # `auto` rather than `install`: the second deploy of the old two-stage
        # split re-installed the canisters the first stage had just installed
        # and died with IC0514 ("canister is not empty"), which also made a
        # resume after any mid-create failure impossible. auto installs an empty
        # canister and upgrades one that already holds code, so a create is
        # re-runnable.
        _progress(f"deploying with icp (-e {args.env}, --mode auto)…")
        _icp(
            ["deploy"] + _base_flags(args) + ["--mode", "auto", "-y"],
            args,
            timeout=900,
        )
        return
