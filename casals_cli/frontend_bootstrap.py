...
def write_icp_project(
    ...
    """Write icp.yaml for the Casals UI asset canister (no build steps).
    ...
    The asset sync plugin only accepts relative, symlink-free dirs, so the dist
    is copied into the project directory. The UI finds its backend through the
    ...
    dists = {"casals_frontend": casals_dist}
    for icp_name, target in dists.items():
