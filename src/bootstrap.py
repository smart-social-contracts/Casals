...
On post_upgrade and when ``file_registry_canister_id`` is wired via
``set_settings``, ensure the orchestra tree contains the Casals section,
System stand, and a registered ``file_registry`` canister when settings
already point at a deployed registry.
...
def _ensure_core_bootstrap() -> dict:
    """Ensure Casals/System + file_registry exist when settings wire a registry.
