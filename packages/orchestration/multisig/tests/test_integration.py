...
class TestMultisigThreshold:
    def test_single_signer_propose_executes(self, multisig_env):
        ...
        proposal_id = parse_nat_output(call(
            ...
            "propose",
            f'(variant {{ AddCommander = record {{ baton_id = principal "{multisig_env["baton_id"]}"; commander = principal "{orch}"; capabilities = vec {{ "propose:managed_upgrade" }} }} }})',
        ))
...
class TestMultisigPolicy:
    def test_set_policy_reaches_baton(self, multisig_env):
        ...
        call(
            multisig_env["multisig_id"],
            "propose",
            f'(variant {{ SetPolicy = record {{ baton_id = principal "{multisig_env["baton_id"]}"; policy_json = "{policy_json}" }} }})',
        )
